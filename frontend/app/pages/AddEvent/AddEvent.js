...
const AddEvent = () => {
    ...
    const handleSubmit = async () => {
        try {
            dispatch(setLoadingInfo('Adding event.'));
            dispatch(setLoading(true));
            const response = await axios.post(`${serverURL}/create-event`, data);
    ...
    return (
        <View style={{ flex: 1, backgroundColor: colors.backgroundColor }}>
            <ScrollView style={{ width: '90%', alignSelf: 'center', marginTop: SIZES.twenty }}>
                ...
                <TextInput style={textInput} placeholder='Enter event title' placeholderTextColor={PLACEHOLDERCOLOR} onChangeText={(value) => { handleChange(value, 'title') }} />
                <TextInput multiline={true} numberOfLines={50} style={[textInput, { textAlignVertical: 'top', minHeight: SIZES.twoHundred, marginTop: SIZES.twenty }]} placeholder='Type event here...' placeholderTextColor={PLACEHOLDERCOLOR} onChangeText={(value) => { handleChange(value, 'body') }} />
