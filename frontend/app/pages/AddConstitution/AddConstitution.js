...
const AddConstitution = () => {
    ...
    const handleSubmit = async () => {
        try {
            dispatch(setLoadingInfo('Adding constitution.'));
            dispatch(setLoading(true));
            const response = await axios.post(`${serverURL}/create-constitution`, data);
    ...
    return (
        <View style={{ flex: 1, backgroundColor: colors.backgroundColor }}>
            <ScrollView style={{ width: '90%', alignSelf: 'center', marginTop: SIZES.twenty }}>
                <Text style={{ fontWeight: 'bold', color: colors.textPrimary }} >Post Constitution</Text>
                <TextInput style={textInput} placeholder='Enter constitution title' placeholderTextColor={PLACEHOLDERCOLOR} onChangeText={(value) => { handleChange(value, 'title') }} />
                <TextInput multiline={true} numberOfLines={50} style={[textInput, { textAlignVertical: 'top', minHeight: SIZES.twoHundred, marginTop: SIZES.twenty }]} placeholder='Type constitution here...' placeholderTextColor={PLACEHOLDERCOLOR} onChangeText={(value) => { handleChange(value, 'body') }} />
